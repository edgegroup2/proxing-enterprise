...
function readLiveClassroomConfig(
  environment = process.env
) {
  const provider =
    normalizeString(
      environment.LIVE_CLASSROOM_PROVIDER
    ).toLowerCase() || 'livekit';
  ...
  return Object.freeze({
    ...
    enabledDefault: parseBoolean(
      environment.LIVE_CLASSROOM_ENABLED_DEFAULT,
      false
    ),

    recordingEnabledDefault: parseBoolean(
      environment
        .LIVE_CLASSROOM_RECORDING_ENABLED_DEFAULT,
      false
    ),
